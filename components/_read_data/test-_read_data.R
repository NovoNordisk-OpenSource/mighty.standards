...
# mocks ------------------------------------------------------------------------
{
  ...
  mock_dm_vaccine <- data.frame(ARM = "A", STUDYID = "S001", USUBJID = "U001", stringsAsFactors = FALSE)
...
test_that("single SDTM domain: renders connector setup and read with column selection", {
  ...
  # EXPECT ---------------------------------------------------------------------
  expect_match(rendered, 'cnt <- connector::connect(config = "_connector.yml")', fixed = TRUE)
  expect_match(rendered, "DM <-  cnt$sdtm$read_cnt(tolower('DM'))", fixed = TRUE)
  expect_match(rendered, "dplyr::select(ARM, STUDYID, USUBJID)", fixed = TRUE)

  # COVERAGE -------------------------------------------------------------------
  inject_connector_mock(component, list(sdtm = list(read_cnt = function(x) mock_dm)))
  component$eval()
})
...
test_that("self domain: renders read without column selection for self, with selection for other", {
  ...
  # EXPECT ---------------------------------------------------------------------
  expect_match(rendered, "ADSL <-  cnt$adam$read_cnt(tolower('ADSL'))", fixed = TRUE)
  expect_match(rendered, "DM <-  cnt$sdtm$read_cnt(tolower('DM'))", fixed = TRUE)
  expect_match(rendered, "dplyr::select(ARM, STUDYID, USUBJID)", fixed = TRUE)
...
test_that("multiple SDTM domains: renders one read block per domain", {
  ...
  # EXPECT ---------------------------------------------------------------------
  expect_match(rendered, "DM <-  cnt$sdtm$read_cnt(tolower('DM'))", fixed = TRUE)
  expect_match(rendered, "DM_VACCINE <-  cnt$sdtm$read_cnt(tolower('DM_VACCINE'))", fixed = TRUE)
  # both domains select the same columns so they can be row-bound in _init_domain
  expect_equal(sum(gregexpr("dplyr::select(ARM, STUDYID, USUBJID)", rendered, fixed = TRUE)[[1]] > 0), 2L)

  # COVERAGE -------------------------------------------------------------------
  inject_connector_mock(
    component,
    list(sdtm = list(read_cnt = function(x) if (x == "dm") mock_dm else mock_dm_vaccine))
  )
  component$eval()
})
...
test_that("ADaM cross-domain: renders read via adam connector with column selection", {
  ...
  # COVERAGE -------------------------------------------------------------------
  inject_connector_mock(component, list(adam = list(read_cnt = function(x) mock_adsl)))
  component$eval()
})
...
test_that("metadata domain: renders read via metadata connector with column selection", {
  ...
  # EXPECT ---------------------------------------------------------------------
  expect_match(rendered, "MDCOL <-  cnt$metadata$read_cnt(tolower('MDCOL'))", fixed = TRUE)
  expect_match(rendered, "dplyr::select(PARAMCD)", fixed = TRUE)

  # COVERAGE -------------------------------------------------------------------
  inject_connector_mock(component, list(metadata = list(read_cnt = function(x) mock_mdcol)))
  component$eval()
})
